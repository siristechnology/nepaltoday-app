...
import { View, Image, Button, SafeAreaView, Text } from '@components'
...
const BottomPlayer = (props) => {
	...
	console.log('printing currentChannel', currentChannel)

	return (
		<View testID="bottomPlayer" style={[styles.playerContainer, { backgroundColor: theme.colors.lightBackground }]}>
			{(currentChannel && currentChannel.artwork && (
				<Image source={{ uri: currentChannel.artwork }} style={styles.imageStyle} />
			...
			<View style={styles.playerInnerView}>
				...
				{currentChannel && <Text style={styles.currentTrack}>{currentChannel.title}</Text>}
			</View>
		</View>
	)
}
